...
function unusableReasonLabel(reason: PresetUnusableReason): string {
  switch (reason) {
    case "manifest_packet_missing":
      return "Linked saved work was removed — re-link before using this saved line.";
    case "manifest_no_published_revision":
      return "Linked saved work has no published version yet — publish before using.";
  }
}
      ...
      <div className="space-y-1">
        ...
        {totalPinnable === 0 ? (
          ...
          <p className="px-2 py-3 text-[11px] text-zinc-500">
            {tradeFilterActive
              ? `No saved work found for ${selectedTrade}${query.trim().length > 0 ? ` matching “${query}”` : ""}.`
              : `No saved work matches for “${query}”.`}
          </p>
        ) : (
      ...
      >
        ...
        <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[9px] uppercase tracking-wider text-zinc-500">
          ...
          {preset.defaultScopePacket ? (
            ...
            >
              Saved work: {preset.defaultScopePacket.displayName}
...
function TradeChipStrip({
  ...
  return (
    <ul
      ...
      aria-label="Filter saved lines and saved work by trade"
    >
      {chips.map((chip) => {
        ...
        return (
          <li key={chip}>
            <button
              ...
              title={
                chip === "All"
                  ? `Show all saved lines and saved work (${count})`
                  : `Show ${chip} saved lines and saved work (${count})`
              }
            >
              {chip}
...
function EmptyState({ totalAvailable }: { totalAvailable: number }) {
  ...
  return (
    <p className="px-2 py-3 text-[11px] text-amber-400">
      {totalAvailable} saved work item{totalAvailable === 1 ? "" : "s"} exist, but none have a
