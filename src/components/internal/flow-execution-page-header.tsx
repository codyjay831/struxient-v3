...
type Props = {
  ...
  flowId: string;
  /** Override for breadcrumbs and back links in production context. */
...
export function FlowExecutionPageHeader({
  ...
  const isProd = !!backLinkOverride;
  const quoteWorkspaceHref = context 
    ? (isProd ? `/quotes/${context.quote.id}` : `/dev/quotes/${context.quote.id}`) 
    : null;
  const flowDetailHref = isProd ? `/flows/${flowId}` : `/dev/flow/${flowId}`;
  const workFeedHref = isProd ? `/flows/${flowId}` : `/dev/work-feed/${flowId}`;

  const breadcrumbSegments = [
    { label: isProd ? "Execution" : "Flows", href: backLinkOverride ?? "/dev/flows" },
    ...(context ? [{ label: context.quote.quoteNumber, href: isProd ? undefined : flowDetailHref }] : []),
    { label: surfaceLabel },
  ];
  ...
  const quickJumpLinks = [
    ...
    ...(quoteWorkspaceHref
      ? [{ label: isProd ? "Quote Workspace" : "Quote workspace", href: quoteWorkspaceHref, variant: "sky" as const }]
      : []),
    { label: isProd ? "Back to Quotes" : "Back to Flows", href: backLinkOverride ?? "/dev/flows" },
    ...(context && !isProd
      ? [
          {
            label: "Job detail",
  ...
  return (
    <header className="mb-6 border-b border-zinc-800 pb-5">
      ...
      <div className="mt-5">
        <InternalQuickJump title={isProd ? "Related Access" : "Continue testing"} links={quickJumpLinks} />
      </div>
    </header>
  );
}
