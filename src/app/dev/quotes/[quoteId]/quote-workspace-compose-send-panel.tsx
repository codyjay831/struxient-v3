...
export function QuoteWorkspaceComposeSendPanel({ latestDraft, canOfficeMutate }: Props) {
  ...
  const runComposePreview = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setLastCompose(null);
        setStalenessTokenForSend(undefined);
        setComposeError(`${body.error?.code ?? "ERROR"}: ${body.error?.message ?? res.status}`);
        return;
      }
      if (!body.data) {
        setComposeError("Unexpected response: missing data.");
        return;
      }
  ...
  const runSend = useCallback(async () => {
    ...
    if (stalenessTokenForSend === undefined) {
      setSendError("Run compose preview first; send requires clientStalenessToken from the compose response.");
      return;
    }
    setSendBusy(true);
    setSendError(null);
    try {
      ...
      if (!res.ok) {
        const extra =
          ...
          : "";
        setSendError(`${body.error?.code ?? "ERROR"}: ${body.error?.message ?? res.status}${extra}`);
        return;
      }
      resetSendLocalState();
      router.refresh();
  ...
  }, [canOfficeMutate, latestDraft, resetSendLocalState, router, stalenessTokenForSend]);
    ...
    return (
      <section className="mb-6 rounded border border-zinc-800 bg-zinc-950/60 p-4 text-sm">
        ...
        <p className="text-xs text-zinc-500">
          Proposal preparation is only available for draft versions. The newest version in history is already locked or sent.
        </p>
      </section>
    );
  }
  ...
  return (
    <section className="mb-6 rounded border border-zinc-800 bg-zinc-950/60 p-4 text-sm">
      <h2 className="mb-1 text-sm font-medium text-zinc-200">Prepare & send proposal</h2>
      <p className="text-xs text-zinc-500">
        Review the proposal composition for v{latestDraft.versionNumber} and send it to the customer.
      </p>
      ...
      : <>
          {!latestDraft.hasPinnedWorkflow ?
            <p className="mt-2 text-xs text-amber-700/90">
              <span className="font-medium text-amber-600/90">A workflow must be selected before sending.</span> 
          ...
          <p className="mt-3 text-[11px] text-zinc-500 leading-relaxed">
            Sending the proposal will <span className="text-zinc-400 font-medium">freeze</span> this version. 
            No further changes can be made to v{latestDraft.versionNumber} after it is sent.
          </p>

          {composeError ? <p className="mt-2 text-xs text-amber-600/90">{composeError}</p> : null}
          {sendError ? <p className="mt-2 text-xs text-amber-600/90">{sendError}</p> : null}
          {summaryLines}
          ...
          <div className="mt-4 border-t border-zinc-800/40 pt-3">
            <details className="text-[10px] text-zinc-600">
              ...
              <div className="mt-2 space-y-2">
                <p>Version ID: <span className="font-mono">{latestDraft.quoteVersionId}</span></p>
                ...
                {lastCompose && (lastCompose.errors.length > 0 || lastCompose.warnings.length > 0) ?
                  <div className="mt-2">
                    <p className="mb-1 text-[9px] uppercase tracking-wider text-zinc-500">Compose messages</p>
